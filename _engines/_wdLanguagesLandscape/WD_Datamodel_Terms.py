...
parsFile = \
   "/home/goransm/Analytics/Wikidata/WD_languagesLandscape/WD_LanguagesLandscape_Config.xml"
...
WD_dump = sqlContext.sql('SELECT id, labels FROM wmf.wikidata_entity WHERE snapshot="' + \
...
WD_dump = WD_dump\
   ...
   .select('language')
WD_dump = WD_dump.groupBy('language')\
   .count()\
...
WD_dump.toPandas()\
   .to_csv(localPath + 'update_Labels.csv', \
   header = True, \
   index = False)

### --- Access WD dump: ALIASES
WD_dump = sqlContext.sql('SELECT id, aliases FROM wmf.wikidata_entity WHERE snapshot="' + \
   wikidataEntitySnapshot + '"')
### --- Cache WD dump
WD_dump.cache()
### --- Explode aliases & select
WD_dump = WD_dump.select('id', explode('aliases').alias("alias", "value"))\
   .select('alias', explode("value"))\
   .select('alias')
WD_dump = WD_dump.groupBy('alias')\
   .count()\
   .orderBy('count', ascending = False)
WD_dump = WD_dump.withColumn("snapshot", lit(wikidataEntitySnapshot))
WD_dump = WD_dump.withColumn("numitems", lit(num_items_general))
WD_dump.toPandas()\
   .to_csv(localPath + 'update_Aliases.csv', \
   header = True, \
   index = False)

### --- Access WD dump: DESCRIPTIONS
WD_dump = sqlContext.sql('SELECT id, descriptions FROM wmf.wikidata_entity WHERE snapshot="' + \
   wikidataEntitySnapshot + '"')
### --- Cache WD dump
WD_dump.cache()
### --- Explode descriptions & select
WD_dump = WD_dump\
   .select('id', explode('descriptions').alias("description", "value"))\
   .select('description')
WD_dump = WD_dump.groupBy('description')\
   .count()\
   .orderBy('count', ascending = False)
WD_dump = WD_dump.withColumn("snapshot", lit(wikidataEntitySnapshot))
WD_dump = WD_dump.withColumn("numitems", lit(num_items_general))
WD_dump.toPandas()\
   .to_csv(localPath + 'update_Descriptions.csv', \
   header = True, \
   index = False)
...
itemFile = \
   'hdfs:///tmp/wmde/analytics/Wikidata/LanguagesLandscape/DataModelTermsCollectedItemsDir/collectedAstronomy.csv'
...
WD_items.cache()
# - explode properties
WD_items = WD_items.select('id', explode('claims').alias('claims'))\
   .select('id', 'claims.mainSnak')
WD_items = WD_items.select('id', 'mainSnak.property', 'mainSnak.dataValue.value')
WD_items = WD_items.filter(WD_items.property == 'P31')\
   .select('id', regexp_extract(col('value'), '(Q\d+)', 1).alias('value'))
...
WD_labels = WD_items.join(WD_dump, ["id"], how='left')\
   .select('language')
WD_labels = WD_labels.groupBy('language')\
   .count()\
   .orderBy('count', ascending = False)
...
# - store
WD_labels.toPandas()\
   .to_csv(localPath + 'update_Labels_ASTRONOMY.csv', \
   header = True, \
   index = False)
...
WD_dump.cache()
WD_dump = WD_dump.select('id', explode('aliases').alias("alias", "value"))\
   .select('id', 'alias', explode("value"))\
   .select('id', 'alias')
# - left_join for astronomy:
WD_aliases = WD_items.join(WD_dump, ["id"], how='left')\
   .select('alias')
WD_aliases = WD_aliases.groupBy('alias')\
   .count()\
   .orderBy('count', ascending = False)
...
# - store
WD_aliases.toPandas()\
   .to_csv(localPath + 'update_Aliases_ASTRONOMY.csv', \
   header = True, \
   index = False)
...
# - Cache WD dump for labels
WD_dump.cache()
WD_dump = WD_dump.select('id', explode('descriptions').alias("description", "value"))\
   .select('id', 'description')
# - left_join for astronomy:
WD_descriptions = WD_items.join(WD_dump, ["id"], how='left')\
   .select('description')
WD_descriptions = WD_descriptions.groupBy('description')\
   .count()\
   .orderBy('count', ascending = False)
...
# - store
WD_descriptions.toPandas()\
   .to_csv(localPath + 'update_Descriptions_ASTRONOMY.csv', \
   header = True, \
   index = False)
...
# - read what is found in subclasses
itemFile = \
   'hdfs:///tmp/wmde/analytics/Wikidata/LanguagesLandscape/DataModelTermsCollectedItemsDir/collectedScientificPapers.csv'
items = sqlContext.read.csv(itemFile, header=True)
items = items.select('id')
# - find what is an SCIENTIFIC PAPER by P31
# - initiate dump for items
WD_items2 = sqlContext.sql('SELECT id, claims FROM wmf.wikidata_entity WHERE snapshot="' + \
   wikidataEntitySnapshot + '"')
# - Cache WD dump for items
WD_items2.cache()
# - explode properties
WD_items2 = WD_items2.select('id', explode('claims').alias('claims'))\
   .select('id', 'claims.mainSnak')
WD_items2 = WD_items2.select('id', 'mainSnak.property', 'mainSnak.dataValue.value')
WD_items2 = WD_items2.filter(WD_items2.property == 'P31')\
   .select('id', regexp_extract(col('value'), '(Q\d+)', 1).alias('value'))
WD_items2 = WD_items2.filter(WD_items2.value == "Q13442814")\
   .select('id')
# - bind with what is found in subclasses
...
WD_dump = WD_dump.select('id', explode('labels').alias("language", "label"))\
   .select('id', 'language')
# - left_join for astronomy:
WD_labels = WD_items2.join(WD_dump, ["id"], how='left')\
   .select('language')
WD_labels = WD_labels.groupBy('language')\
   .count()\
   .orderBy('count', ascending = False)
...
# - store
WD_labels.toPandas()\
   .to_csv(localPath + 'update_Labels_SCIENTIFICPAPERS.csv', \
   header = True, \
   index = False)
...
# - Cache WD dump for labels
WD_dump.cache()
WD_dump = WD_dump.select('id', \
   explode('aliases').alias("alias", "value"))\
   .select('id', 'alias', explode("value"))\
   .select('id', 'alias')
# - left_join for astronomy:
WD_aliases = WD_items2.join(WD_dump, ["id"], how='left')\
   .select('alias')
WD_aliases = WD_aliases.groupBy('alias')\
   .count()\
   .orderBy('count', ascending = False)
...
# - store
WD_aliases.toPandas()\
   .to_csv(localPath + 'update_Aliases_SCIENTIFICPAPERS.csv', \
   header = True, \
   index = False)
...
WD_dump.cache()
WD_dump = WD_dump.select('id', \
   explode('descriptions').alias("description", "value"))\
   .select('id', 'description')
# - left_join for astronomy:
WD_descriptions = WD_items2.join(WD_dump, ["id"], how='left')\
   .select('description')
WD_descriptions = WD_descriptions.groupBy('description')\
   .count()\
   .orderBy('count', ascending = False)
...
# - store
WD_descriptions.toPandas()\
   .to_csv(localPath + 'update_Descriptions_SCIENTIFICPAPERS.csv', \
   header = True, \
   index = False)
...
WD_dump.cache()
WD_dump = WD_dump.select('id', \
   explode('labels').alias("language", "label"))\
   .select('id', 'language')
# - left_anti_join:
WD_labels  = WD_dump.join(WD_items, on=['id'], how='left_anti')\
   .select('language')
WD_labels = WD_labels.groupBy('language')\
   .count()\
   .orderBy('count', ascending = False)
...
# - store
WD_labels.toPandas()\
   .to_csv(localPath + 'update_Labels_EVERYTHINGMINUS.csv', \
   header = True, \
   index = False)
...
WD_dump = WD_dump.select('id', \
   ...
   .select('id', 'alias')
# - left_anti_join:
WD_aliases  = WD_dump.join(WD_items, on=['id'], how='left_anti')\
   .select('alias')
WD_aliases = WD_aliases.groupBy('alias')\
   .count()\
   .orderBy('count', ascending = False)
...
# - store
WD_aliases.toPandas()\
   .to_csv(localPath + 'update_Aliases_EVERYTHINGMINUS.csv', \
   header = True, \
   index = False)
...
WD_dump = WD_dump.select('id', explode('descriptions').alias("description", "value"))\
   .select('id', 'description')
# - left_anti_join:
WD_descriptions = WD_dump.join(WD_items, ["id"], how='left_anti')\
   .select('description')
WD_descriptions = WD_descriptions.groupBy('description')\
   .count()\
   .orderBy('count', ascending = False)
...
# - store
WD_descriptions.toPandas()\
      .to_csv(localPath + 'update_Descriptions_EVERYTHINGMINUS.csv', \
      header = True, \
      index = False)
