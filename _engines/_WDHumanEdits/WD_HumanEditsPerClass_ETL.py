...
parsFile = \
   "/home/goransm/Analytics/Wikidata/WD_HumanEdits/wdHumanEditsPerClass_Config.xml"
...
WD_dump = sqlContext.sql('SELECT id, claims.mainSnak, labels \
...
WD_dump = WD_dump.select('id', \
   col('mainSnak.property').alias('property'), \
   col('mainSnak.datavalue.value').alias('value'), \
   explode('labels').alias("language", "label"))
# produce labs (en)
labs = WD_dump.select('id', 'label', 'language')\
   .where(col('language').isin("en"))\
   .select('id', 'label')\
   .distinct()
labs = labs.filter(labs['id'].rlike("Q\d+"))
WD_dump = WD_dump.filter(WD_dump.property == 'P31')
WD_dump = WD_dump.select(col("id").alias("item"), \
   col("value").alias("wd_class"))\
   .distinct()
WD_dump = WD_dump\
   .withColumn('wd_class', regexp_extract(col('wd_class'), '(Q\d+)', 1))
...
wmwh = sqlContext.sql('SELECT page_title, event_user_is_bot_by \
                      ...
                      mwwikiSnapshot + '"')
wmwh = wmwh\
   .withColumn("bot_name", array_contains(col("event_user_is_bot_by"), "name"))
wmwh = wmwh\
   .withColumn("bot_group", array_contains(col("event_user_is_bot_by"), "group"))
wmwh = wmwh.select('page_title', 'bot_name', 'bot_group')
wmwh = wmwh.withColumn("human", when((col("bot_name") == True) | \
   (col("bot_group") == True), 0).otherwise(1))
wmwh = wmwh.withColumn("bot", when((col("bot_name") == True) | \
   (col("bot_group") == True), 1).otherwise(0))
wmwh = wmwh.select(col('page_title').alias('itemhist'), 'human', 'bot')
wmwh = wmwh.groupBy('itemhist')\
   .agg(sum("human"),sum("bot"))
wmwh = wmwh.select(col('itemhist'), \
   col("sum(human)").alias("human_edits"), \
   col("sum(bot)").alias("bot_edits"))
wmwh = wmwh.orderBy('human_edits', ascending = False)

# - Left join: WD_dump on wmwh
wmwh = wmwh.join(WD_dump, \
   wmwh.itemhist == WD_dump.item, \
   how='left')
...
humanBotItems = humanBotItems\
   .withColumn("total_edits", col("human_edits")+col("bot_edits"))
humanBotItems = humanBotItems\
   .withColumn("human_to_bot_ratio", col("human_edits")/col("bot_edits"))
humanBotItems = humanBotItems\
   .withColumn("human_ratio", col("human_edits")/col("total_edits"))
humanBotItems = humanBotItems\
   .withColumn("bot_ratio", col("bot_edits")/col("total_edits"))
humanBotItems = humanBotItems\
   .withColumn("human_percent", col("human_ratio")*100)
humanBotItems = humanBotItems\
   .withColumn("bot_percent", col("bot_ratio")*100)
# labels to humanBotItems
humanBotItems = humanBotItems\
   .join(labs, \
   humanBotItems.item == labs.id, \
   how='left')\
   .drop('id')\
   .orderBy('human_ratio', ascending = False)
# save: humanBotItems
fileName = "humanBotItems.csv"
humanBotItems.repartition(30)\
   .write.option("quote", "\u0000")\
   .format('csv')\
   .mode("overwrite")\
...
humanBotClasses = humanBotClasses\
   .groupBy('wd_class')\
   .agg(sum("human_edits"), sum("bot_edits"))
humanBotClasses = humanBotClasses\
   .select(col('wd_class'), \
   col("sum(human_edits)").alias("human_edits"), \
   col("sum(bot_edits)").alias("bot_edits"))
humanBotClasses = humanBotClasses\
   .withColumn("total_edits", col("human_edits") + col("bot_edits"))
humanBotClasses = humanBotClasses\
   .withColumn("human_to_bot_ratio", col("human_edits")/col("bot_edits"))
...
humanBotClasses = humanBotClasses\
   .join(labs, \
   humanBotClasses.wd_class == labs.id, \
   how='left')\
   .drop('id')\
   .orderBy('human_ratio', ascending = False)
# save: humanBotClasses
fileName = "humanBotClasses.csv"
humanBotClasses.repartition(30)\
   .write.option("quote", "\u0000").format('csv')\
   .mode("overwrite")\
   .save(hdfsPath + fileName)

# Number and percent of items edited by Humans, per Class
wmwh = wmwh\
   .withColumn("human_edited", when((col("human_edits") > 0), 1).otherwise(0))
wmwh = wmwh.select("wd_class", "human_edited")
wmwh = wmwh.groupBy("wd_class")\
   .agg(sum("human_edited"), count("wd_class"))
wmwh = wmwh.select(col('wd_class'), \
   col("sum(human_edited)").alias("human_edited_items"), \
   col("count(wd_class)").alias("num_items"))
wmwh = wmwh\
   .withColumn("proportion_items_touched", col("human_edited_items")/col("num_items"))
wmwh = wmwh\
   .withColumn("percent_items_touched", (col("human_edited_items")/col("num_items"))*100)
# - join labels
wmwh = wmwh.join(labs, \
   wmwh.wd_class == labs.id, \
   how='left')\
   .drop('id')\
   .drop('item')
wmwh = wmwh\
   .orderBy(['proportion_items_touched', 'num_items'], ascending = [0, 0])
...
wmwh = sqlContext.sql('SELECT page_title, event_user_id, event_user_is_bot_by \
                      FROM wmf.mediawiki_history \
                      WHERE event_entity="revision" AND \
                      event_type="create" AND \
                      wiki_db="wikidatawiki" AND \
...
wmwh = wmwh\
   .withColumn("bot_group", array_contains(col("event_user_is_bot_by"), "group"))
wmwh = wmwh\
   .select('page_title', 'event_user_id', 'bot_name', 'bot_group')
wmwh = wmwh\
   .withColumn("human", when((col("bot_name") == True) | \
   (col("bot_group") == True), 0).otherwise(1))
wmwh = wmwh.filter(wmwh.human == 1)
wmwh = wmwh.select('page_title', 'event_user_id')\
   .distinct()
wmwh = wmwh.select('page_title')\
   .groupBy('page_title')\
   .count()
# - join WD_dump to wmwh
wmwh = wmwh\
   .join(WD_dump, \
   wmwh.page_title == WD_dump.item, \
   how='left')\
   .drop('item')
wmwh = wmwh.filter("wd_class is not NULL")
# - median number of unique human contributors by class
wmwh_class = wmwh.select('wd_class', 'count')\
   .groupBy('wd_class')\
   .agg(expr('percentile(count, array(0.5))')[0].alias('median'), \
   count('wd_class').alias('num_items')) 
wmwh_class = wmwh_class\
   .join(labs, \
   wmwh_class.wd_class == labs.id, \
   how='left')\
   .drop('id')\
   .orderBy('median', ascending = False)
# save: wmwh_class
fileName = "classMedianEditors.csv"
wmwh_class.repartition(30)\
   .write.option("quote", "\u0000")\
   .format('csv')\
   .mode("overwrite").save(hdfsPath + fileName)
