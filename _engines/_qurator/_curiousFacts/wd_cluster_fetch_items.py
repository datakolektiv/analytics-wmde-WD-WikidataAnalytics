...
parsFile = "/home/goransm/Analytics/Qurator/CuriousFacts_Product/wd_cluster_fetch_items.xml"
...
WD_items = WD_items.filter((WD_items["id"].rlike('Q\d+')) & ((WD_items.property == 'P31') |(WD_items.property == 'P279')))
# - filter by: subclasses list
WD_items = WD_items.filter(WD_items['value'].isin(subclasses))
WD_items = WD_items.select('id', 'value')
...
WD_dump = WD_dump.select('id', 'mainSnak.property', regexp_extract(col('mainSnak.dataValue.value'), '(Q\d+)', 1).alias('value'))
...
# - join to WD_items
WD_dump = WD_dump.withColumnRenamed('value', 'propValue')
WD_items = WD_items.select('id').join(WD_dump.select('id', 'propValue'), on=['id'], how='left')
WD_items = WD_items.na.drop(subset=['propValue'])
...
referenceClasses = sqlContext.read.csv(referenceClassFile, header=True)
referenceClasses = referenceClasses.select('subclass')
# - referenceClasses to list + add tclass 
referenceClasses = referenceClasses.toPandas()
referenceClasses =  referenceClasses['subclass'].tolist()
referenceClasses.append(rclass)
# - unique elements only
referenceClasses = list(set(referenceClasses))

# - find what is in classes by P31
# - initiate dump for items
WD_refitems = sqlContext.sql('SELECT id, claims FROM wmf.wikidata_entity WHERE snapshot="' + wikidataEntitySnapshot + '"')
# - cache WD dump for items
WD_refitems.cache()
# - explode properties
WD_refitems = WD_refitems.select('id', explode('claims').alias('claims')).select('id', 'claims.mainSnak')
WD_refitems = WD_refitems.select('id', 'mainSnak.property', 'mainSnak.dataValue.value')
WD_refitems = WD_refitems.select('id', 'property', regexp_extract(col('value'), '(Q\d+)', 1).alias('value'))
# - keep items only and P31 or P279
WD_refitems = WD_refitems.filter((WD_refitems["id"].rlike('Q\d+')) & ((WD_refitems.property == 'P31') |(WD_refitems.property == 'P279')))
# - filter by: referenceClasses list
WD_refitems = WD_refitems.filter(WD_refitems['value'].isin(referenceClasses))
WD_refitems = WD_refitems.select('id', 'value')

### --- Section 4. All WD_items where propValue is not in WD_refitems
WD_refitems = WD_refitems.withColumnRenamed('id', 'propValue').select('propValue')
WD_items  = WD_items.join(WD_refitems, on=['propValue'], how='left_anti')
