...
app_server <- function( input, output, session ) {
  ...
  ### --- Config File
  params <- XML::xmlParse(system.file("app/www/config_WD_ExternalIdentifiersDashboard.xml", 
                                      package = "WDExternalIdentifiersDashboard"))
  ...
  identifiers <- wd_IdentifiersFrame %>% 
    dplyr::select(property, label, used)
  ...
  ### --- output$globalMap
  output$globalMap <- plotly::renderPlotly({
    
    # plotFrame
    plotFrame <- wd_Map %>% 
      dplyr::select(D1, D2, label, usage)
    ...
    idsFrame <- data.frame(sId = identifiers$identifier,
                           ...
                           stringsAsFactors = F)
    plotFrame <- dplyr::left_join(plotFrame, idsFrame, 
  ...
  output$similarityGraph <- plotly::renderPlotly({
    ...
    withProgress(message = 'Generating Network.', detail = "Please be patient.", value = 0, {
      ...
      Xn <- L[,1]
      Yn <- L[,2]
      a <- rowSums(dplyr::select(wd_CoOccurence, -Identifier))
      f <- data.frame(summa = a, 
                      id = wd_CoOccurence$Identifier, 
                      stringsAsFactors = F) %>%
        dplyr::arrange(desc(summa))
      vsnames <- data.frame(id = vs$name, 
                            prop = stringr::str_extract(vs$name, "P[[:digit:]]+"),
                            stringsAsFactors = F)
      f$id <- as.character(f$id)
      vsnames$prop <- as.character(vsnames$prop)
      vsnames <- dplyr::left_join(vsnames, f, by = c("prop" = "id"))
      vsnames$prop <- NULL
      # - {plotly}
  ...
  output$overlapDT <- DT::renderDataTable({
    ...
    dFrame_overviewDT <- reactive({
      sel <- stringr::str_extract(input$selectId, "P[[:digit:]]+")
      ...
      if (dim(dFrame)[1] == 0) {
        ...
        dFrame <- dplyr::arrange(dFrame, desc(dFrame[, 2]))
        labs <- dplyr::select(wd_IdentifiersFrame, 
                       property, label)
        labs <- labs[!duplicated(labs), ]
        labs$label <- paste0(labs$label, " (", labs$property, ")")
        dFrame <- dplyr::left_join(dFrame, labs, 
    ...
    # - DT:
    DT::datatable(dFrame_overviewDT(), 
              filter = 'top',
              options = list(
                pageLength = 50,
                width = '100%',
  ...
  output$localSimilarityGraph <- plotly::renderPlotly({
  ...
  output$identifierClassList <- DT::renderDataTable({
    dFrame <- wd_IdentifiersFrame %>% 
      dplyr::filter(classLabel %in% input$selectClass)
    ...
    dFrame <- dplyr::select(dFrame, Identifier)
  ...
  output$identifierSimilarityGraph <- plotly::renderPlotly({
    ...
    if (!is.null(dSet)) {
      ...
      idNet2 <- identifierConnected10 %>% 
        dplyr::filter(Outgoing %in% friends)
  ...
  output$identifierReport <- renderText({
    ...
    res <- httr::GET(url = URLencode(APIquery))
    # - External Identifers to a data.frame
    if (res$status_code == 200) {
      # - jsonlite::fromJSON
      pStatements <- jsonlite::fromJSON(rawToChar(res$content), simplifyDataFrame = T)
    }
    ...
    idC <- stringr::str_extract(idClasses, "Q[[:digit:]]+")
  ...
  output$examplesDT <- DT::renderDataTable({
    ...
    withProgress(message = 'Contacting WDQS.', detail = "Please be patient.", value = 0, {
      ...
      if (res$status_code == 200) {
        # - jsonlite::fromJSON
        example <- jsonlite::fromJSON(rawToChar(res$content), simplifyDataFrame = T)
      }
      ...
      exClass <- data.table::rbindlist(exClass)
      example <- example %>% 
        dplyr::select(item, itemLabel, value)
      example <- example[!duplicated(example), ]
      example <- dplyr::left_join(example, exClass, by = "item")
      example$itemId <- stringr::str_extract(example$item, "Q[[:digit:]]+")
      example$item <- paste0('<a href = "', example$item, '" target = "_blank">', 
                             paste0(example$itemLabel, " (", example$itemId, ")"), 
                             '</a>')
      dFrame <- example %>% dplyr::select(
        item, value, classes)
      colnames(dFrame) <- stringr::str_to_title(colnames(dFrame))
